    ...
    try {
      ...
      if (!user) {
        ...
        user = await User.create({
          username: (
            googleUser.displayNameusername || googleUser.email.split("@")[0]
          )
            .toLowerCase()
            .replace(" ", ""),
...
router.post("/login", upload.none(), async (req, res) => {
  try {
    ...
    if (passwordMatch) {
      ...
      io.to(username).emit("new_notification", { notification });
    } else {
...
router.post("/register", upload.none(), async (req, res) => {
  try {
    ...
    if (username == "" || name == "" || email == "" || password == "") {
      return res.status(400).json({ error: "Null data on form" });
    }
  ...
  try {
    ...
    if (req.body.isAdmin || req.body.isActive) {
      return res
        ...
        .json({
          message:
            "isAdmin veya isActive alanlarını güncellemeye yetkiniz yok.",
        });
    }
    ...
    res
      .status(200)
      .json({ message: "Kullanıcı bilgileri güncellendi.", updatedUser });
...
router.post("/follow/:id", tokenControl, upload.none(), async (req, res) => {
  try {
    ...
    if (!followingUser) {
      return res
        .status(404)
        .json({ message: "Takip edilen kullanıcı bulunamadı." });
    }
...
router.post("/unfollow/:id", tokenControl, upload.none(), async (req, res) => {
  try {
    ...
    if (!isAlreadyFollowing) {
      return res
        .status(400)
        .json({ message: "Bu kullanıcıyı zaten takip etmiyorsunuz." });
    }
