...
router.post("/shareArticle", tokenControl, upload.none(), async (req, res) => {
  try {
    ...
    await article.save();
    res.status(200).json({
      message: "Makale başarıyla paylaşıldı."
    });
...
router.put("/updateArticle/:id", tokenControl, upload.none(), async (req, res) => {
  try {
    ...
    const {
      content
    } = req.body; // Kullanıcının kimliğini talep gövdesinden alın
...
router.delete("/deleteArticle/:id", tokenControl, upload.none(), async (req, res) => {
  try {
    ...
    if (article.authorId != userId) {
      return res.status(403).json({
        message: "Bu makaleyi silmeye yetkiniz yok."
      });
    }
  ...
  try {
    ...
    if (article.authorId == userId) {
      return res.status(400).json({
        message: "Kendi makalenizi beğenemezsiniz."
      });
    }
    const user = await User.findByPk(userId);
    const author = await User.findByPk(article.authorId);
    ...
    if (likedShares) {
      ...
      res.status(500).json({
        message: "Makale beğenirken bir hata oluştu."
      });
    }
  } catch (error) {
    //console.log("ARTICLES/error : ", error.message);
